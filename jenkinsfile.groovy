        ...
        stage('Checkout Code') {
            steps {
                script {
                    ...
                    def branch = 'ibtihel'
                    withCredentials([usernamePassword(credentialsId: '', usernameVariable: 'GITHUB_USERNAME', passwordVariable: 'GITHUB_TOKEN')]) {
                        checkout([$class: 'GitSCM',
                            branches: [[name: "${branch}"]],
                            userRemoteConfigs: [[url: "${repoUrl}", credentialsId: '']]
                        ])
                    }
                }
            }
